...
# Make oat++ to link with winsock libraries. For some reason cmake won't trigger the MINGW variable.
...
# Build and install
RUN cd /oatpp && mkdir build && cd build && cmake -DOATPP_BUILD_TESTS=OFF .. && make -j$(nproc) && make install && cd ../.. && rm -rf oatpp
